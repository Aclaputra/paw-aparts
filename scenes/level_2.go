...
type Level2Scene struct {
	ecs        *ecs.ECS
	lastUpdate time.Time
	elapsed    float64
}

func NewLevel2Scene() *Level2Scene {
	return &Level2Scene{
		ecs: createECSLevel2(),
	}
}

func (l2 *Level2Scene) Update(g *game.Game) {
	if l2.lastUpdate.IsZero() {
		l2.lastUpdate = time.Now()
	}

	now := time.Now()
	dt := now.Sub(l2.lastUpdate).Seconds()
	l2.lastUpdate = now
	...
	if ent, ok := components.Dialogue.First(l2.ecs.World); ok {
		menu := components.Dialogue.Get(ent)
		if menu.NextSceneTriggered {
			menu.NextSceneTriggered = false

			systems.DisposeMusic(l2.ecs)
			g.ChangeScene(NewResultMenuScene("Level 2", l2.elapsed, "Thanks"))
		}
	}
}

func (l2 *Level2Scene) Draw(screen *ebiten.Image) {
